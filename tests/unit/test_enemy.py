...
def test_enemy_drop_low_chance(enemy: Enemy) -> None:
    enemy.drop_items = [Item(name="Sword", damage_boost=10, defense_boost=5, health_boost=20)]

    with patch("entities.enemy.random") as mock_random:
        mock_random.return_value = 0.2
        assert enemy._Enemy__enemy_drop() is None

def test_enemy_drop_high_chance(enemy: Enemy) -> None:
    enemy.drop_items = [Item(name="Sword", damage_boost=10, defense_boost=5, health_boost=20)]
