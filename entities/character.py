...
class Character(Entity):
    ...
    def __init__(
        self, level: int = 1, inventory: list = [], character_name: str = "Hero"
    ):
        super().__init__(character_name, level)
    ...
    def add_item(self, item: Item) -> bool:
        if len(self.inventory) + 1 > MAX_INVENTORY_SLOTS:
            print(
                f"Você não pode adicionar mais de {MAX_INVENTORY_SLOTS} itens ao inventário."
            )
            return False
    ...
    def inventory_handler(self) -> None:
        ...
        while user_input != 3:
            ...
            if user_input == 1:
                self.equip_item(item_position)
            elif user_input == 2:
                self.unequip_item(item_position)
            else:
